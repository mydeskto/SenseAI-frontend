...
import { UserAppSidebar } from "../SimpleUser/Compoenets/User-app-sidebar";
import { AnimatePresence } from "framer-motion";
import UserLeft from '../SimpleUser/Compoenets/UserLeft'
...
const UserHome = () => {
    ...
    const {open , setIsLogin, setName ,setEmail , setLastName ,  setImg ,  setAllowChat, setAllowDoc  } = useUserContext();
  ...
  const handleSignIn = () => {
    ...
    const fetchUser = async () => {
      try {
        ...
        console.log(response.data.user.isAllowCHat, response.data.user.isAllowDoc)

        const data = await response.data.user;
        console.log(data)
        if(data.isAllowCHat){ 
          setAllowChat(true) 
          console.log("if conditon",data.isAllowCHat)
        }else if(!data.isAllowCHat ){
          setAllowChat(false) 
          console.log("if conditon",data.isAllowCHat)

        }
        if(data.isAllowDoc === true){ 
          setAllowDoc(true) 
          console.log("if conditon for doc",data.isAllowDoc)
          console.log()
        }else if(data.isAllowDoc === false){
          setAllowDoc(false) 
          console.log("if conditon for doc",data.isAllowDoc)
        }
    ...
    <SidebarProvider>
      <div className="flex w-full h-full">
        ...
        <div className="flex-1 flex flex-col h-screen ">
          ...
          <AnimatePresence>{open && < UserLeft/>}</AnimatePresence>
